...
function preload() {
  ...
  bastionModel = loadModel('bastion.obj', true); // Load the MTL file along with the OBJ file
}

function setup() {
  createCanvas(windowWidth, windowHeight, WEBGL); // Enable 3D mode
  noStroke(); // Remove visible vertices for smoothness
  directionalLight(255, 255, 255, 0, 1, -1); // Add a directional light
...
function draw() {
  ...
  // Position the model based on the mouse position
